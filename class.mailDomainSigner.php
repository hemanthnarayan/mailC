...
class mailDomainSigner{
  ...
  private $__app_ver					= "0.2-20110415";
   ...
   * @author Ahmad Amarullah
   */
  public function __construct($private_key,$d,$s,$private_key2,$d2,$s2){  
    // Get a private key
    ...
    // Get a private key
    $this->pkid2 = openssl_pkey_get_private($private_key2);
  ...
  public function getDKIM2($h,$_h,$body) {
    ...
    $_dkim = "DKIM-Signature: ".
                "v=1; ".                  // DKIM Version
                "a=rsa-sha1; ".           // The algorithm used to generate the signature "rsa-sha1"
                "s={$this->s2}; ".         // The selector subdividing the namespace for the "d=" (domain) tag
                "d={$this->d2}; ".         // The domain of the signing entity
                "l={$_l}; ".              // Canonicalizated Body length count
                "t={$_t}; ".              // Signature Timestamp
                "c=relaxed/relaxed; ".    // Message (Headers/Body) Canonicalization "relaxed/relaxed"
                "h={$h}; ".               // Signed header fields
    ...
    // Sign Canonicalization Header Data with Private Key
    openssl_sign($_unsigned, $_signed, $this->pkid2, OPENSSL_ALGO_SHA1);
